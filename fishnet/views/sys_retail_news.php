...
<section class="primary">
	<div class="header-a" style="border-left: 12px solid <?=val($revision['revision_text']['color'], '#000000')?>">
		...
		<h2>
			<?=anchor('monthly/'.$page_id, val($title, 'Untitled Page'))?> >
...
<!--            <span class="counter">--><?//=$i + 1?><!--</span>-->
			<h2 class="b"><?=anchor("/article/".$news[$i]['page_id'], $news[$i]['title'])?></h2>
			<?php
			...
			<p><?=$news[$i]['revision_text']->related_info?></p>
...
</section> <!--/ #primary TODO: Create page parts for this content and the right column -->

<aside class="secondary">
	<div class="section-a">
		<h2 class="c">
			<span style="background-color: <?=val($revision['revision_text']['color'], '#000000')?>"></span>
			<?=anchor("calendar/".val($revision['revision_text']['calendar']), "Retail Calendar")?>
		</h2>
	...
	<div class="section-a">
		<h2 class="c"><label for="byDate">Jump to a month</label></h2>
