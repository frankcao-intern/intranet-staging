...
// ================================================
	...
	//Contruct iBrowser object
	window.ib = {
		//-------------------------------------------------------------------------
		// set image attributes
		getImgParam: function (oImageElement, wArgs) {
			...
			if (oImageElement.hspace >= 0) { // (-1 when not set under gecko for some reason)
				wArgs.hspace = oImageElement.hspace;
			}
			if (oImageElement.vspace >= 0) { // // (-1 when not set under gecko for some reason)
				wArgs.vspace = oImageElement.vspace;
			}
		...
		getLinkParam: function (oLinkElement, wArgs) {
		...
		//-------------------------------------------------------------------------
		// set image caption
		setCaption: function (elm,chkCaption, caption, captionClass) {
		...
		getElementByAttributeValue: function (node, element_name, attrib, value) {
		...
		//-------------------------------------------------------------------------
		// set attributes
		setAttrib: function (element, name, value, fixval) {
		...
		insertNodeAtSelection: function (win, insertNode) { // Gecko
		...
		getNextSiblingByName: function (node, name) {
	...
	function iBrowser_open() {
		...
		if (ib.isMSIE) { // IE
			var rArgs = showModalDialog(ib.baseURL, wArgs, 'dialogHeight:530px; dialogWidth:635px; scroll: yes; menubar: no; toolbar: no; resizable: yes; status: no;');
