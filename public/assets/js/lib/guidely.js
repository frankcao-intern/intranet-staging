...
var guidely = (function($){
	...
	return {
		...
		, _createGuide: function (id, options) {
			...
			if (this._guides.length === id) {
				if (this._guides.length > 1){
					this._createPrevButton ().appendTo (controls);
				}

				this._createDoneButton ().appendTo (controls);
			}else if (id === 1) {
		...
		, _setGuidePosition: function (number, content) {
			var offset, left, top, docWidth, docHeight;
			...
			if (content.outerHeight() + offset.top > docHeight){
				top = (offset.top - content.outerHeight()) + 30;
			}else{
				top = offset.top + 15;
			}
...
if (define){
	define('guidely', function(){ return guidely; });
}
